...
public class GlobalExceptionHandler {
	...
	@ExceptionHandler(GuiaDespachoPublishException.class)
	public ResponseEntity<ErrorResponse> handleGuiaDespachoPublishException(
			GuiaDespachoPublishException ex, WebRequest request) {

		log.error("Error al publicar en RabbitMQ: {}", ex.getMessage());

		ErrorResponse error = ErrorResponse.builder().timestamp(LocalDateTime.now())
				.status(HttpStatus.BAD_GATEWAY.value()).error("RabbitMQ Publish Failed")
				.message(ex.getMessage()).details("Verifique que el contenedor de RabbitMQ esté disponible")
				.path(getPath(request)).build();
